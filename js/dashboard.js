...
function buildSidebarNav(role) {
    ...
    // Add other roles (owner, staff, delivery) here if needed
...
async function loadRolePanel(role, targetPanelKey = 'default') {
    ...
    switch (targetPanelKey) {
        ...
        default:
            effectivePanelKey = role === 'admin' ? 'users' : 'defaultPanel'; // Adjust default panel as needed
            panelModuleToLoad = loadAdminPanel; // Default to admin panel for admin
            break;
    }

    if (typeof panelModuleToLoad === 'function') {
        panelModuleToLoad(panelRoot, panelTitle, db, auth); // navContainer is no longer passed
