...
/**
 * Fetches and loads the sales data for the selected date into the form.
 */
...
/**
 * Calculates and displays the total sales based on the input fields.
 */
...
/**
 * Saves the entered sales data to Firebase.
 */
...
export function loadPanel(root, panelTitle) {
    ...
    panelRoot.querySelector('#daily-count-tbody')?.addEventListener('input', (e) => {
        if (e.target.classList.contains('daily-input')) calculateRow(e.target.closest('tr'));
    });
