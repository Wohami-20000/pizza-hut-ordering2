// auth.js - FINAL, COMPLETE, AND CORRECTED
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Initialize International Telephone Input with CORS Fix ---
    const iti = window.intlTelInput(elements.signupPhoneInput, {
        ...
        geoIpLookup: cb => {
            fetch("https://api.allorigins.win/raw?url=https://ipapi.co/json")
                ...
                .catch(() => cb("us")); // Fallback to US on error
        }
    });
    ...
    class PasswordInput extends HTMLElement {
            ...
            this.shadowRoot.innerHTML = `
                <style>
                    ...
                    input[type="password"], input[type="text"] {
                        ...
                        color: #231F20; transition: border-color 0.2s, box-shadow 0.2s; box-sizing: border-box;
                    }
                    input[type="password"]:focus, input[type="text"]:focus {
                        outline: none; border-color: #FFC72C; box-shadow: 0 0 0 2px rgba(255, 199, 44, 0.5);
                    }
                    .toggle-password { position: absolute; top: 50%; right: 0.75rem; transform: translateY(-50%); cursor: pointer; color: #9ca3af; }
                    .strength-meter { height: 6px; width: 100%; background-color: #e5e7eb; border-radius: 3px; overflow: hidden; margin-top: 2px; }
                    .strength-bar { height: 100%; width: 0%; background-color: #ef4444; transition: width 0.3s, background-color 0.3s; }
                    .strength-text { font-size: 0.75rem; text-align: right; min-height: 1rem; }
                    .error-message { color: #dc2626; font-size: 0.875rem; min-height: 1rem; display: none; }
        ...
        checkStrength() {
            const pass = this.passwordInput.value; let score = 0;
            if (pass.length >= 8) score++; if (/[A-Z]/.test(pass)) score++; if (/[0-9]/.test(pass)) score++; if (/[^A-Za-z0-9]/.test(pass)) score++;
            const strengthLevels = { 1: { width: '25%', color: '#ef4444', text: "Weak" }, 2: { width: '50%', color: '#f97316', text: "Medium" }, 3: { width: '75%', color: '#f59e0b', text: "Good" }, 4: { width: '100%', color: '#22c55e', text: "Strong" } };
            if (pass.length === 0) { this.strengthBar.style.width = '0%'; this.strengthText.textContent = ''; return; }
            const level = strengthLevels[score] || { width: '10%', color: '#ef4444', text: "Very Weak" };
            this.strengthBar.style.width = level.width; this.strengthBar.style.backgroundColor = level.color; this.strengthText.textContent = level.text; this.strengthText.style.color = level.color;
        }
        validatePasswords() { const match = this.passwordInput.value === this.confirmPasswordInput.value; this.passwordError.classList.toggle('visible', !match && this.confirmPasswordInput.value.length > 0); return match; }
    ...
    const handleAuthError = (error, formType) => {
        ...
        let emailErrorId = `${formType}-email-error`, passwordErrorId = `${formType}-password-error`, generalErrorId = `${formType}-error-message`;
        switch (error.code) {
            ...
            case 'auth/invalid-credential':
                ...
                break;
            case 'auth/wrong-password': displayError(passwordErrorId, 'Incorrect password.'); break;
            case 'auth/invalid-email': displayError(emailErrorId, 'Please enter a valid email address.'); break;
            case 'auth/weak-password': displayError(passwordErrorId, 'Password must be at least 6 characters.'); break;
            case 'auth/email-already-in-use': displayError(emailErrorId, 'This email is already registered.'); break;
            case 'auth/popup-closed-by-user':
                message = 'Sign-in window closed. Please try again.';
                displayError(generalErrorId, message);
                break;
            default: displayError(generalErrorId, message); break;
        }
    };
    ...
    elements.sendResetLinkBtn.addEventListener('click', () => {
        ...
        auth.sendPasswordResetEmail(email)
            .then(() => { successEl.textContent = 'Password reset link sent! Check your inbox.'; })
            .catch(error => { errorEl.textContent = error.code === 'auth/user-not-found' ? 'No account found with this email.' : 'An error occurred.'; });
    });
